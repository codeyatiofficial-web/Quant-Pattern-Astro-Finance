...
function getColor(value: number, mode = "dma") {
  if (mode === "dma") {
    ...
    if (value <= 10) return { bg: "var(--bg-secondary)", text: "var(--text-muted)", border: "var(--border-active)" };
  ...
  return { bg: "var(--bg-secondary)", text: "var(--text-muted)", border: "var(--border-active)" };
}
...
export default function NiftyHeatmap() {
  ...
  if (loading && data.length === 0) {
    return (
      <div style={{
        ...
        background: "var(--bg-card)", color: "var(--text-primary)", padding: "40px",
        borderRadius: "16px", border: "1px solid var(--border-subtle)", marginBottom: "24px",
        textAlign: "center"
      }}>
        <div style={{ fontSize: "14px", color: "var(--text-muted)", marginBottom: "8px" }}>Loading Nifty 50 Heatmap...</div>
        <div style={{ fontSize: "11px", color: "var(--text-secondary)" }}>Fetching real market data for 50 stocks (may take 15-30s on first load)</div>
      </div>
    );
  }
  ...
  if (error && data.length === 0) {
    return (
      <div style={{
        fontFamily: "'JetBrains Mono', 'Fira Code', monospace",
        background: "var(--bg-card)", color: "var(--accent-red)", padding: "40px",
        borderRadius: "16px", border: "1px solid rgba(239,68,68,0.3)", marginBottom: "24px",
        textAlign: "center"
      }}>
        <div style={{ fontSize: "14px", marginBottom: "8px" }}>Heatmap Error</div>
        <div style={{ fontSize: "11px", color: "var(--text-muted)" }}>{error}</div>
        <button onClick={refreshData} style={{
          marginTop: 12, padding: '6px 16px', fontSize: 11, fontWeight: 700,
          background: 'var(--bg-secondary)', border: '1px solid var(--border-active)', borderRadius: 6,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <div>
            <div style={{ fontSize: "10px", color: "var(--text-muted)", letterSpacing: "3px", textTransform: "uppercase" }}>NSE INDEX</div>
            <div style={{ fontSize: "22px", fontWeight: "700", letterSpacing: "-0.5px", color: "var(--text-primary)" }}>
              NIFTY 50 <span style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: "400" }}>HEATMAP</span>
            </div>
          </div>
          <div style={{
            ...
            <div style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "2px" }}>INDEX CHANGE</div>
        ...
        <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
          {[
            ...
            { label: "UNCHANGED", value: 50 - advancers - decliners, color: "var(--text-muted)" },
          ].map(stat => (
            <div key={stat.label} style={{ textAlign: "center" }}>
              <div style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "2px" }}>{stat.label}</div>
      ...
      {data.length > 0 && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: "32px", flexWrap: "wrap", width: "100%" }}>
            {/* Short Term */}
            <div style={{ display: "flex", gap: "16px", flex: 1, minWidth: "250px", borderRight: "1px solid var(--border-subtle)", paddingRight: "16px" }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "1px", marginBottom: "4px" }}>TODAY'S BEST BUY (SCORE)</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                  <span style={{ fontSize: "15px", fontWeight: "700", color: "#4ade80" }}>{bestBuyToday?.symbol}</span>
                  <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>{bestBuyToday?.score}</span>
                </div>
              </div>
              <div style={{ width: "1px", height: "30px", background: "var(--border-subtle)", margin: "auto 0" }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "1px", marginBottom: "4px" }}>TODAY'S TOP SELL (SCORE)</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                  <span style={{ fontSize: "15px", fontWeight: "700", color: "#f87171" }}>{bestSellToday?.symbol}</span>
                  <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>{bestSellToday?.score}</span>
                </div>
              </div>
            </div>
            {/* Long Term */}
            <div style={{ display: "flex", gap: "16px", flex: 1, minWidth: "250px" }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "1px", marginBottom: "4px" }}>DEEP VALUE BUY (200 DMA)</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                  <span style={{ fontSize: "15px", fontWeight: "700", color: "#4ade80" }}>{bestBuyLongTerm?.symbol}</span>
                  <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>{bestBuyLongTerm?.dma > 0 ? "+" : ""}{bestBuyLongTerm?.dma}%</span>
                </div>
              </div>
              <div style={{ width: "1px", height: "30px", background: "var(--border-subtle)", margin: "auto 0" }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "1px", marginBottom: "4px" }}>STRETCHED SELL (200 DMA)</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                  <span style={{ fontSize: "15px", fontWeight: "700", color: "#f87171" }}>{bestSellLongTerm?.symbol}</span>
                  <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>+{bestSellLongTerm?.dma}%</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "2px", marginRight: "4px" }}>VIEW:</span>
        {[
          ...
          <button key={v.id} onClick={() => setViewMode(v.id)} style={{
            ...
            border: `1px solid ${viewMode === v.id ? "#3b82f6" : "var(--border-subtle)"}`,
            borderRadius: "5px", padding: "5px 12px",
            color: viewMode === v.id ? "#60a5fa" : "var(--text-muted)",
            cursor: "pointer", fontSize: "10px", letterSpacing: "1px",
          }}>
            {v.label}
          </button>
        ...
        <span style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "2px", marginRight: "4px" }}>COLOR BY:</span>
        {[
          ...
          <button key={m.id} onClick={() => setHeatmapMode(m.id)} style={{
            ...
            border: `1px solid ${heatmapMode === m.id ? "#8b5cf6" : "var(--border-subtle)"}`,
            borderRadius: "5px", padding: "5px 12px",
            color: heatmapMode === m.id ? "#c4b5fd" : "var(--text-muted)",
            cursor: "pointer", fontSize: "10px", letterSpacing: "1px",
          }}>
            {m.label}
          </button>
        ...
        <span style={{ fontSize: "9px", color: "var(--text-muted)", letterSpacing: "2px", marginRight: "4px" }}>SIZE BY:</span>
        {["dma", "weight", "volume", "iv"].map(s => (
          <button key={s} onClick={() => setSortBy(s)} style={{
            background: sortBy === s ? "rgba(16,185,129,0.2)" : "transparent",
            border: `1px solid ${sortBy === s ? "#10b981" : "var(--border-subtle)"}`,
            borderRadius: "5px", padding: "5px 12px",
            color: sortBy === s ? "#34d399" : "var(--text-muted)",
              ...
              return (
                ...
                >
                  ...
                  <div style={{
                    ...
                    height: "2px", background: SECTOR_COLORS[stock.sector] || "var(--border-active)"
                  }} />
              ...
              return (
                <div key={sec.sector} style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ fontWeight: "700", fontSize: "12px", color: "var(--text-primary)", flex: 1 }}>
                      {sec.sector.toUpperCase()}
                    </span>
                    <span style={{ fontSize: "9px", color: "var(--text-muted)" }}>{sec.count} stocks · {sec.totalWeight.toFixed(1)}% weight</span>
                    <span style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px" }}>
            <div>
              ...
              <div style={{ fontSize: "18px", fontWeight: "700", color: "var(--text-primary)" }}>{selectedStock.symbol}</div>
              <div style={{ fontSize: "11px", color: "var(--text-muted)" }}>{selectedStock.name}</div>
            </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: "9px", color: "var(--text-muted)", textTransform: "uppercase" }}>{heatmapMode === "dma" ? "200 DMA DIST" : "DAY CHANGE"}</div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
            {[
              ...
              <div key={stat.label} style={{
                background: "var(--bg-secondary)", borderRadius: "6px", padding: "8px",
                border: "1px solid var(--border-subtle)"
              }}>
                <div style={{ fontSize: "8px", color: "var(--text-muted)", letterSpacing: "1px" }}>{stat.label}</div>
                <div style={{ fontSize: "13px", fontWeight: "700", color: stat.color || "var(--text-primary)", marginTop: "2px" }}>
                  {stat.value}
                </div>
              </div>
            ))}
          ...
          <div style={{ marginTop: "12px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
              <span style={{ fontSize: "8px", color: "var(--text-muted)" }}>RSI INDICATOR</span>
              <span style={{ fontSize: "8px", color: selectedStock.rsi > 70 ? "#f87171" : selectedStock.rsi < 30 ? "#4ade80" : "var(--text-muted)" }}>
                {selectedStock.rsi > 70 ? "OVERBOUGHT" : selectedStock.rsi < 30 ? "OVERSOLD" : "NEUTRAL"}
              </span>
            </div>
            <div style={{ background: "var(--border-subtle)", borderRadius: "4px", height: "6px", position: "relative" }}>
              <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "3px" }}>
              <span style={{ fontSize: "7px", color: "var(--text-muted)" }}>0</span>
              <span style={{ fontSize: "7px", color: "var(--text-muted)" }}>30</span>
              <span style={{ fontSize: "7px", color: "var(--text-muted)" }}>70</span>
              <span style={{ fontSize: "7px", color: "var(--text-muted)" }}>100</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        <span style={{ fontSize: "8px", color: "var(--text-muted)", letterSpacing: "2px", marginRight: "4px" }}>
        ...
        {heatmapMode === "dma" && [
          ...
          { label: "-5% TO +10%", bg: "var(--bg-secondary)", text: "var(--text-muted)" },
        ...
        {heatmapMode === "change" && [
          ...
          { label: "0%", bg: "var(--bg-secondary)", text: "var(--text-muted)" },
        ...
        <div style={{ marginLeft: "auto", fontSize: "8px", color: "var(--text-muted)" }}>
